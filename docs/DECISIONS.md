...
- 飛行、碰撞、固定戰區與操作預設由研究文件明列；變更數值前須同步 spec 與驗收。此條目僅記錄設計選擇，不能作為 PvP 已實作或上線證據。
...
- 本機預測採 120 筆固定步時間線，權威 tick 與控制包序號分離；耗盡時取消輸入並同步，不悄悄丟棄歷史後繼續回放。此為有限視覺預測，命中與勝負仍只由房主決定，效能與延遲待實測。
- 基本 sync／resume／exchange 與取消確認提前至共用基礎及 US1，避免後期重連任務成為前期必要依賴。完整測試入口包含房間、HUD、控制、傳輸、橋接與 HTTP；純 Python 測試獨立保留。
- 正式部署之後才取得的證據另用文件 PR 交付，遵守每次修改同步及已合併分支清理要求。本次只有設計修正，不代表已執行這些功能測試或部署。
