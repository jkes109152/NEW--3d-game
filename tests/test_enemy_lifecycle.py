...
class EnemyLifecycleTests(unittest.TestCase):
    def test_T06_unique_drop_and_airborne_damage(self):
        b = battle()
        b.aircraft[list(b.aircraft)[0]].kind = 'MANPOWER_SUPPORT'
        b.aircraft[list(b.aircraft)[0]].position = V3(-18,20,100)
        b.damage_aircraft(list(b.aircraft)[0],1)
        b.damage_aircraft(list(b.aircraft)[0],1)
    ...
    def test_zero_drop_can_complete(self):
        b = battle()
        b.aircraft[list(b.aircraft)[0]].kind='FAST'
        b.damage_aircraft(list(b.aircraft)[0],1)
        b.check_outcome()
