...
class CampaignV2Tests(unittest.TestCase):
    def test_reward_failure_save_retries_identity_and_manual_cursor(self):
        with isolated_roots() as (root,_):
            ...
            old=b.attempt_id;b=start_prepared(app);self.assertNotEqual(b.attempt_id,old);b.fail('player');app.settle();app.show_menu();self.assertEqual(app.cursor,(1,1))
            b=start_prepared(app);self.assertEqual(b.level.level_id,'1-1');self.assertEqual(app.transaction('reward',old,a=1,b=1,A=2)['reason'],'phase')
