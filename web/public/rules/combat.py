...
class LockState:
    ...
    def update(self, dt, candidates, alive, aiming, multi, duration):
        ...
        if not multi:
            retaining=self.current in alive and (self.current in candidates or self.progress.get(self.current,0)>0)
            selected=self.current if retaining else (candidates[0] if candidates else None)
...
def target_in_box(player, position, size=.21, aspect=16/9, fov=90):
    ...
    return abs(delta.dot(right)*scale)<=min(size/2,aspect/2)+1e-9 and abs(delta.dot(up)*scale)<=min(size/2,.5)+1e-9
