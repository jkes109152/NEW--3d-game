...
let desktop = false;
try {
  await access(root + '../air_defense/state.py');
  desktop = true;
} catch {}
if (desktop)
  for (const name of modules)
    await copyFile(
      root + '../air_defense/' + name + '.py',
      root + 'public/rules/' + name + '.py',
    );
...
for (const name of [
  ...
  await copyFile(
    root + 'node_modules/pyodide/' + name,
    root + 'public/runtime/' + name,
  );
if (desktop) {
  await cp(root + '../assets/audio', root + 'public/audio', {
    recursive: true,
  });
  await cp(root + '../assets/textures', root + 'public/textures', {
    recursive: true,
  });
}
